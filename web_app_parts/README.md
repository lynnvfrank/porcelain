...
These scripts are the **original Claudia mobile chat UI** (sidebar, bubbles, fork, avatars). They concatenate into `locus/static/web_app.js`.

Edit these files; then build:

```bash
python locus/build_web_app.py
```

Restart `locus/server.py` and hard-refresh. The shell HTML is generated from Claudia Core’s `mobile_orchestrator_api.py` via `locus/tools/emit_claudia_web_shell.py` when that template changes.
